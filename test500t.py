...
class MultiMeshApp(QtWidgets.QMainWindow):
    ...
    def create_scene(self):
        ...
        geometries = [
            pv.Sphere(radius=size / 2, theta_resolution=3, phi_resolution=3),  # Сфера
            pv.Cube(x_length=size, y_length=size, z_length=size),  # Куб
            pv.Cone(radius=size / 2, height=size, resolution=3),  # Конус
        ]
    ...
    def on_selection_changed(self, index):
        if index < 0 or self.centers is None:
            return
        ...
        self.plotter.add_mesh(
            box, color="white", style="wireframe", line_width=2, name="highlight"
        )
